...
if(document.body.classList.contains('hiringPage')){
	...
	submitClick.addEventListener('click', () =>{
		...
		let userFormPhone = document.forms["userForm"]["phoneNumber"].value;
		let userFormTextArea = document.forms["userForm"]["formTextArea"].value;
		validateForm(userFormAge,userFormPhone,userFormTextArea)
})
	
	
function validateForm(userformage,userformphone,formtextarea){
	if (userformage<21||userformage>99){
	...
	if (formtextarea.length >30){
		alert("max of 30 characters is allowed");
		return false;
	}
	console.log(document.forms["userForm"].values)
